...
| RP A100/B75 merged checkpoint | Uploaded and verified | `thomasjvu/rally-e2b-rp-merged-upload` completed on 2026-05-12 after the `HF_TOKEN` Kaggle secret was added. The private HF repo `thomasjvu/rally-2b-rp-a100-b75-merged` now contains the intended hard-boundary v8 checkpoint; `scaled_lora_merge.json` verifies `applied: 205` and `scale: 0.75`. |
...
3. Keep `thomasjvu/rally-2b-rp-a100-b75-merged` as the provenance checkpoint for the promoted Rally RP text package; it now verifies `applied: 205` and `scale: 0.75`.
