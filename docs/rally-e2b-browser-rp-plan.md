...
| Direct Heretic full | Template-composed, validated, cleaned, uploaded | `thomasjvu/rally-2b` was deleted/recreated private to clear the older full package history and stale README, then uploaded from `alkahestai/rally-e2b-direct-full-compose` version 3. Current private repo is `thomasjvu/rally-2b`, HF commit `6131f825540facbc8efaa98b837db79850bfcc4f`, with q4f16 text + image sessions only and no stale public README. The Kaggle package report is `ok=true`, warnings are empty, and q4f16 vision artifacts were copied from the refreshed `alkahestai/rally-e2b-export-prep` version 4 template. |
