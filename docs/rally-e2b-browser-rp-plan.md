...
The first thomasjvu hard-boundary SFT rerun included the intended rows (`stage_b_gemma_hard_boundary_repeats=120`) but still failed every minor gate in the four-candidate scorecard sweep. Its training log showed only about 5.7M trainable parameters and Unsloth was attaching gradients to `vision_tower.encoder`, so the next rerun changes the Rally LoRA target to a language-only regex for the inner projection linears: `.*language_model.*\.(q_proj|k_proj|v_proj|o_proj|gate_proj|up_proj|down_proj)\.linear$`. The intermediate attempt to target the projection wrappers directly failed because PEFT cannot wrap `Gemma4ClippableLinear`.
