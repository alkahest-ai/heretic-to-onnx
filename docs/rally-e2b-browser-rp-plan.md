...
The first thomasjvu hard-boundary SFT rerun included the intended rows (`stage_b_gemma_hard_boundary_repeats=120`) but still failed every minor gate in the four-candidate scorecard sweep. Its training log showed only about 5.7M trainable parameters and Unsloth was attaching gradients to `vision_tower.encoder`, so the next rerun discovers the exact language-model inner projection linears after loading Gemma4 and passes that explicit list to Unsloth/PEFT. Direct wrapper targets failed because PEFT cannot wrap `Gemma4ClippableLinear`, and a regex string failed because Unsloth's Gemma4 helper requires a list or tuple. Some Unsloth wrappers omit `language_model` in module names, so discovery now falls back to non-vision projection `.linear` modules when the preferred names are absent.
