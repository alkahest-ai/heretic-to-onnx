...
- The direct full package remains the older `thomasjvu/rally-2b@51cb78d3ac4a95d9999d28f1ff72e0240730793a` on HF, but the repo is now private. `rally_e2b_direct_full_compose` version 1 validated a replacement direct full package on Kaggle with upload disabled, and the same compose lane is being rerun under `alkahestai` from refreshed export-prep output so it can be downloaded from the currently authenticated Kaggle account.
