...
- The direct full package remains the older `thomasjvu/rally-2b@51cb78d3ac4a95d9999d28f1ff72e0240730793a` on HF, but `rally_e2b_direct_full_compose` version 1 validated a replacement direct full package on Kaggle with upload disabled. Making the existing HF repo private hit the HF private-storage quota, so keep it diagnostic-only until a replacement/storage decision is made.
