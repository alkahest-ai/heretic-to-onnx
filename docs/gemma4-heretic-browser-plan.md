...
- The older RP merged HF checkpoint remains at `thomasjvu/rally-2b-rp-a100-b75-merged@3f2f180e1abea16d236e43e79b1e8454a1a5f168`; it is private and structurally valid, but its `scaled_lora_merge.json` shows `applied: 148`. The current hard-boundary v8 checkpoint was recreated and validated on Kaggle by `rally_e2b_rp_merged_upload` version 3 with `applied: 205`, but HF upload is still pending because the Kaggle `HF_TOKEN` secret was unavailable and local disk is too low for a 10.25 GB checkpoint transfer.
...
- The direct full text+image ONNX package is now complete from the template-compose Kaggle path and uploaded privately at `thomasjvu/rally-2b@6131f825540facbc8efaa98b837db79850bfcc4f`. The repo was deleted/recreated before upload to clear the older full-package history and stale README; it now contains q4f16 text + image sessions only.
