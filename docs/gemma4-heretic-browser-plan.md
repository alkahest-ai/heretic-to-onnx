...
- The current hard-boundary v8 merged checkpoint is uploaded privately at `thomasjvu/rally-2b-rp-a100-b75-merged` with `scaled_lora_merge.json` showing `applied: 205` and `scale: 0.75`.
- The previous direct/RP full packages were deleted because they were stale text+image-only packages and failed local Gemma4 multimodal smoke. The replacement lane is full text+image+audio and must be built from the Lisper-compatible template package.
- The browser runtime now mirrors the Lisper q4f16 path: Gemma4 media prompts pass media placeholders before text, pass image arrays to the positional processor call, read audio at the processor sampling rate, and patch ONNX Runtime feeds that expect float16. `thomasjvu/lisper-gemma4-e2b-audio-onnx-q4f16` passes local image and audio smoke with this runtime.
- Next Kaggle step: refresh `thomasjvu/rally-e2b-export-prep` so its optimized template includes `onnx/audio_encoder_q4f16.*`, then rerun the direct and RP full compose notebooks with `RALLY_UPLOAD=1` to recreate private `thomasjvu/rally-2b` and `thomasjvu/rally-2b-rp` as full text+image+audio packages.
