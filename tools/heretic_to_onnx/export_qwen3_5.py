...
from .qwen3_5_export_codegen import build_qwen3_5_export_contract, render_qwen3_5_export_runner
...
def export_qwen3_5(
    ...
    *,
    ...
    output_dir = (layout.export_raw / "qwen3_5").resolve()
    runner_path = (layout.export_raw / "export_qwen3_5_runner.py").resolve()
    contract_path = (layout.reports / "qwen3_5-export-contract.json").resolve()
    runner_report_path = (layout.reports / "qwen3_5-export-execute.json").resolve()
    ...
    if mode in {"script", "execute"}:
        ...
        report.notes.append(f"generated Qwen3.5 export runner at {runner_path}")
    ...
    if mode == "plan":
        report.notes.extend(
            [
                "Plan mode validates the Qwen3.5 browser contract and writes the export contract only.",
        ...
        if not contract.ok:
            report.warnings.append("execute mode was requested, but the Qwen3.5 export contract is not currently compatible")
        else:
